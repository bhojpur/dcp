...
while ! curl -s --fail https://storage.googleapis.com/dcp-ci-builds > /dev/null; do
    ((iterations++))
    if [ "$iterations" -ge 30 ]; then
        echo "Unable to hit googleapis.com/dcp-ci-builds"
        exit 1
    fi
    sleep 1
done
...
iterations=0
curl -s --fail https://storage.googleapis.com/dcp-ci-builds/dcp-$(head -n 1 $2).sha256sum
while [ $? -ne 0 ]; do
    ((iterations++))
