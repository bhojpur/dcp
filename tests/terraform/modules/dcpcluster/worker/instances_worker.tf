resource "aws_instance" "worker" {
  ...
  provisioner "file" {
    source = "join_dcp_agent.sh"
    destination = "/tmp/join_dcp_agent.sh"
  }
  provisioner "remote-exec" {
    inline = [
      "chmod +x /tmp/join_dcp_agent.sh",
      "sudo /tmp/join_dcp_agent.sh ${var.node_os} ${var.install_mode} ${var.dcp_version} ${local.master_ip} ${local.node_token} ${self.public_ip} \"${var.worker_flags}\" ${var.username} ${var.password} ",
    ]
  }
}
