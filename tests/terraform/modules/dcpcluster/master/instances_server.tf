...
resource "aws_instance" "master" {
  ...
  provisioner "file" {
    source = "install_dcp_master.sh"
    destination = "/tmp/install_dcp_master.sh"
  }
  provisioner "remote-exec" {
    inline = [
      "chmod +x /tmp/install_dcp_master.sh",
      "sudo /tmp/install_dcp_master.sh ${var.node_os} ${var.create_lb ? aws_route53_record.aws_route53[0].fqdn : "${aws_instance.master.public_ip}"} ${var.install_mode} ${var.dcp_version} ${var.cluster_type} ${self.public_ip} \"${data.template_file.test.rendered}\" \"${var.server_flags}\"  ${var.username} ${var.password}",
    ]
  }
...
resource "aws_instance" "master2-ha" {
  ...
  provisioner "file" {
    source               = "join_dcp_master.sh"
    destination          = "/tmp/join_dcp_master.sh"
  }
  provisioner "remote-exec" {
    inline = [
      "chmod +x /tmp/join_dcp_master.sh",
      "sudo /tmp/join_dcp_master.sh ${var.node_os} ${var.create_lb ? aws_route53_record.aws_route53[0].fqdn : "${aws_instance.master.public_ip}"} ${var.install_mode} ${var.dcp_version} ${var.cluster_type} ${self.public_ip} ${aws_instance.master.public_ip} ${local.node_token} \"${data.template_file.test.rendered}\" \"${var.server_flags}\" ${var.username} ${var.password}",
    ]
  }
}
